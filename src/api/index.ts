// 模拟 API 请求
import { Article, Comment, PaginatedResponse, Tag, User } from '../types';
...
const mockArticles: Article[] = [
  {
    ...
    cover: 'https://images.pexels.com/photos/1181467/pexels-photo-1181467.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2',
    views: 261,
  ...
  },
  {
    ...
    intro: '探索 React 18 中的并发渲染、自动批处理和 Suspense 服务器组件等革命性功能',
  ...
  },
  {
    ...
    intro: '掌握 TypeScript 中的映射类型、条件类型、类型守卫和工具类型，提升代码质量',
    cover: 'https://images.pexels.com/photos/4164418/pexels-photo-4164418.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2',
    views: 235,
...
// 模拟 API 函数
export const getArticles = async (page = 1, pageSize = 10): Promise<PaginatedResponse<Article>> => {
  // 模拟网络延迟
...
export const getCommentsByArticleId = async (articleId: number): Promise<Comment[]> => {
  // 模拟网络延迟
...
export const likeArticle = async (articleId: number): Promise<{ success: boolean }> => {
  // 模拟网络延迟
  await new Promise(resolve => setTimeout(resolve, 300));
  ...
  if (article) {
    ...
    article.likes = article.isLiked === 1 ? article.likes + 1 : article.likes - 1;
  }
