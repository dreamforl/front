...
import { Article, listRes, User } from '../types';
...
export const getArticles = async (page = 1, pageSize = 10): Promise<listRes<Article>> =>
  zwFetch('/api/article', { query: { page, pageSize } });

export const getArticleById = async (id: number) => zwFetch<Article>(`/api/article/${id}`);
