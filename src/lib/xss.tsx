// 定义允许的 SVG 标签白名单
const allowedSvgTags = ['svg', 'circle', 'rect', 'line', 'path', 'text', 'polyline'] as const;
// 定义允许的 SVG 属性白名单
...
function filterSvg(svgString: string): string {
  ...
  function filterNode(node: Node): void {
    if (node.nodeType === Node.ELEMENT_NODE) {
      ...
      if (!allowedSvgTags.includes(tagName as (typeof allowedSvgTags)[number])) {
        // 如果标签不在白名单内，移除该节点
      ...
      attributes.forEach(attr => {
        if (!allowedSvgAttributes.includes(attr.name as (typeof allowedSvgAttributes)[number])) {
...
const SafeSvgRenderer: React.FC<SafeSvgRendererProps> = ({ svg, ...restProps }) => {
